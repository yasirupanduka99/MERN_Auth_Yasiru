...
const AllItems = () => {
  ...
  useEffect(() => {
        ...
        await axios
          ...
          .catch((err) => {
            if (err.response) {
              ...
              setError(
                "☠️ Error occurred while processing your get request." +
                  err.message
    ...
    //check for user available. then only getAllItems funnction will call in useEffect.
    if (user) {
      getAllItems(); // This line calls the getAllItems() function when the component mounts | mounts means when component start
    } else {
      setError("🔒 :: You must be logged in ☝🏽");
    }
  }, [dispatch, user]); //The empty dependency array ([]) as the second argument to useEffect indicates that this effect should only run once when the component mounts.
        ...
        await axios
          ...
          .catch((err) => {
            if (err.response) {
              ...
              // If no response received
              setError(
                "☠️ Error occurred while processing your delete request." +
                  err.message
              ); // Set a generic error message
            }
          });
